...
public class ExpressionData {
			...
			if(genes.get(i)>=0)
				...
				if (t == null || t.getRowCount() == 0) {
					...
					while (genes.contains(sparseGeneLabels.getTuple(j).getInt(
							"actualId")))
						j++;// mientras estï¿½ en genes, probamos con la sig.
	...
	public class LoadTask extends SwingWorker<Integer, Void> {
		...
		public int loadFromFile() {
			...
			try {
				...
				} else // Si no estï¿½n invertidas las primeras columnas tienen
						// informaciï¿½n sobre los genes
			...
			if (invert)
				levels = invert(levelsi); // OJO: Para el caso de que estï¿½n
											// invertidos (SynTReN por ejemplo)
			else
				levels = levelsi;
			t1 = System.currentTimeMillis();
			// System.out.println("T2) inversiï¿½n "+(t1-t2)/1000);
	...
	public void loadMicroarray(String path, boolean invert, int rowHeader,
		...
		if (invert) {
			numGenes = levelsi.getColumnCount();// Al revï¿½s te lo digo para
												// que me entiendas
			numConditions = levelsi.getRowCount();
		} else {
		...
		geneLabels.addColumn("name", String.class);
		geneLabels.addColumn("id", int.class);
		geneLabels.addColumn("rowRank", int.class);// Orden en el que serï¿½n
													// pintadas
		int row = 0;
		int sparseRow = 0;
		int step = 1;
