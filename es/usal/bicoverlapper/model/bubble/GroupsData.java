...
public class GroupsData {
	...
	public GroupsData(String filePath, ExpressionData md, Session sesion) throws IOException {
		...
		//Carlos, para contar el n�mero diferentes de genes
		...
		cont = 0;
		cad = br.readLine(); // La primera l�nea contiene el n�mero de
								// biclusters
				...
				if (isSize)// size info, no interesa
				{
					...
					if (areGenes) {
						...
						//	System.out.println(c);
							//se controla el n�mero de genes le�dos
							genesLeidos.add(c);
							//en caso de superar una determinada cifra y de no estar notificado, se notificar� en la sesi�n
							//pero se seguir� leyendo
		...
		// Ahora tenemos que hacer la proyecci�n
		if (bubbles.size() > 0) {
			doProjection();
	...
	public void buildGraphFromProjection() {
		// A�adimos todos los identificadores
		nodes = new Table();
	...
	/**
	 * A�ade nuevas filas, correspondientes a otros ficheros de resultados. Se
	 * distinguen unos de otros mediante la condici�n "resultType"
	 */
	...
	private void buildNodeTable(String genes, String conditions) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (i == 0) {
				...
				// System.out.println("Lista de tama�o "+listag.size());
			}
			if (listag != null && listag.size() > 0 && listac != null
					&& listac.size() > 0) {
				row = nodes.addRow();
	...
	private void buildEdgeTable(String adjacency) {
		...
		// A la larga, a�adiendo un weightGenes y un weightConditions
		...
		// System.out.println("N�mero de aristas "+(row-1));
	}
