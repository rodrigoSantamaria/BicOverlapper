...
public class WordCloudDiagram extends Diagram implements ChangeListener,
	...
	public void setWords(Graphics2D g2) {
		...
		//por defecto end est� a true porque es posible que no se encuentren palabras a dibujar
		//si no se tiene en cuenta esto se entrar�a en bucle infinito al incrementar la escala hasta infinito
		//se pondr� end a false cuando se encuentra en "1) Primera vuelta" al menos una palabra
		boolean end = true;
		...
		for (String w : sortedWords) {
			...
			if (w.length() > 0 && nW != null) {
				...
				//al menos se ha encontrado una palabra, as� que ya se podr� entrar al bucle que incrementa el tama�o
				end = false;
		...
		//s�lo para probar por qu� sale infinity
		...
		// 3) Calculamos el tama�o del as palabras
		...
		do {
			...
			//			System.out.println("\n\n\n\n\n\nPOSITIVE_INFINITY ALCANZADO AL HACER scale*1.5 y scale val�a "+scale);
					}
					
					scale *= 1.5;
				} else {
					if(scale/1.2 == Double.POSITIVE_INFINITY){
			//			System.out.println("\n\n\n\n\n\nPOSITIVE_INFINITY ALCANZADO AL HACER scale/1.2 y scale val�a "+scale);
					}					
