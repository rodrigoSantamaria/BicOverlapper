...
public class Overlapper extends JProcessingPanel implements GeneRequester {
	...
	float areaInc = 2.0f;// Este hay que ir multiplic�ndolo y dividir por lo
	...
	boolean fullDrawing = false; // Para imprimir la imagen entera sin ce�irnos
									// a los m�rgenes de la ventana (para fotos)
	...
	GraphPoint2D vf = new GraphPoint2D();// Vector para la aplicaci�n de fuerzas
											// en el layout
	...
	public void resize(int w, int h) {
		...
		// size(screenWidth,screenHeight);//S�lo en caso de usarlo desde el
		// panel, el P3D parece dar problemas
		// size(screenWidth,screenHeight,P3D);//TODO: S�lo en caso de usarlo
		// desde el panel, el P3D parece dar problemas
		// size(screenWidth,screenHeight,OPENGL);//S�lo en caso de usarlo desde
	...
	public synchronized void paintComponent(Graphics g) {
		...
		if (g != null && gfinal != null) {
			...
			if (gr != null && backBuffer != null) {
				...
				// que hacer los peque�os cambios (s�lo las l�neas est�n en
				// dicha imagen)
	 ...
	 */
	// Ahora en est� mal hecho, ya que se aplican las fuerzas antes de asegurar
	// que todas las fuerzas se le
	// han aplicado, pero quiero ver si eso afecta sustancialmente, ya que si no
	// lo hace en una iteraci�n lo hace para la
	...
	private void applyLayout(ForcedNode n) {
		...
		n.setForce(nullVector); // una vez cambiada la posici�n, estas fuerzas
								// ya no act�an sobre el nodo
		if (g.dualNodes != null && g.dualNodes.size() > 0) {
	...
	public synchronized void doLinLogLayout() {
		...
		int numMutant = (int) (0.6 * numPop * g.getNodes().size() * 2);// un 40%
																		...
																		// y (el
																		// *2)
		int mutFactor = 20;// la variaci�n es de un valor aleatorio en [-10,10]
							// pixeles
	...
	public synchronized void doDualLayout() {
		...
		for (int i = 0; i < g.dualNodes.size(); i++) // N(N-1)/2 complexity
			...
			n.setForce(nullVector); // una vez cambiada la posici�n, estas
									// fuerzas ya no act�an sobre el nodo
		}// for (each node)
	...
	/**
	 * M�todo para que cuando el cursor salga de la ventana, se deseleccione todo y no se mantenga repintado como si estuviera el cursor encima.
	 * Si esto ocurre ser�a una situaci�n err�nea, ya que es falso que el cursor siga encima.
	 * Esto ocurr�a al cambiar de ventana y al salir por un borde de la aplicaci�n desde un pol�gono seleecionado.
	 */
	...
	Graph buildOrderedRadialGraph() {
		...
		RadialCluster[] clu = orderRadialClusters();// A�ade un campo orden a
													// cada cluster
		...
		while (e < 100000 && areaInc < maxArea)// Espacio m�nimo que quiero que
												// tenga cada cluster
		{
			increaseOverview(2);
		...
		for (int l = 0; l < clu.length; l++)// Para cada cluster
		{
			...
			if (row % 2 == 0)
				x = screenWidth * (areaInc - 1) / 2 + row * cellWidth
						+ cellWidth / 2; // direcci�n ->
			else
				x = screenWidth * (areaInc - 1) / 2 + screenWidth - row
						* cellWidth - cellWidth / 2; // direcci�n <-
	...
	Graph buildOrderedCompleteGraph() {
		...
		removeByThreshold();
		System.out.println("No solapados tenemos " + numClusters);
		MaximalCluster[] clu = orderClusters();// A�ade un campo orden a cada
												// cluster
		...
		for (int i = 0; i < clustersToRemove.size(); i++) {
			...
			for (int k = 0; k < lista.size(); k++)// Para cada nodo del cluster
			{
				...
				if (threshold > 0) {// Tenemos que comprobar que no quitamos un
									// nodo que est� en otros clusters
									// TODO: No s� si es mejor dejar el nodo con
									// la informaci�n de que pertenece a varios
									// clusters o quitarla tambi�n
	...
	void removeNonSelected(Selection bs) {
															...
															// quitar
		{
			...
			for (int k = 0; k < lista.size(); k++)// Para cada nodo del cluster
			{
				...
				for (int j = 0; j < list.length; j++) {
										...
										// queda
					{
						if (map.containsKey("cluster" + j)
								&& !c.label.equals("cluster" + j))// est� este
																	// nodo
						{
				...
				if (toRemove) // se quita solo si no est� en alg�n cluster
								// seleccionado
				{
					nodesToRemove.add(nr);
					g.getNodes().remove(nr.getLabel());// y a s� mismo
	...
	public boolean buildGraph() {
		...
		insertDetails();
		Runtime.getRuntime().gc();// Al final del constructor quiz�s sea buena
									// idea
	...
	// ------------------------------
	/*
	 * TODO: Muchos de estos par�metros, aunque est�n en biclusVis, hay que
	 * asegurar que se usan m�s en esta clase que en las inferiores
	 */
